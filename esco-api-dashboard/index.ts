...
const escoApiLambdaId = new pulumi.StackReference(`${org}/esco-api/${stack}`).getOutput("escoApiLambdaId");
...
const dashboard = new aws.cloudwatch.Dashboard(`${projectName}-${stack}`, {
  ...
  dashboardBody: resources.apply(([escoApiLambdaId]) =>
    JSON.stringify({
      widgets: [
        {
          type: "text",
        ...
        },
        {
          ...
          type: "metric",
          properties: {
            metrics: [["AWS/Lambda", "Invocations", "FunctionName", escoApiLambdaId, { id: "m1", region: region, color: "#3e82e5", label: "ESCO API (Sum)" }]],
            view: "timeSeries",
            stacked: false,
            region: region,
            stat: "Sum",
